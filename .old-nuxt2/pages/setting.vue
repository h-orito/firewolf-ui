...
export default class VillageSetting extends Vue {
  ...
  private reset(): void {
    ...
    this.ageLimit =
      tagCodes.length <= 0
        ? ''
        : (tagCodes.find((t) => t.startsWith('R')) ?? '')

    const restricts = rules.message_restrict.restrict_list
    const normal = restricts.find(
      (r) => r.type.code === MESSAGE_TYPE.NORMAL_SAY
    )!
    ...
    const lovers = restricts.find(
      (r) => r.type.code === MESSAGE_TYPE.LOVERS_SAY
    )!
