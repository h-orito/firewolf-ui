...
export default function CharachipDetailPage({ searchParams }: CharachipDetailPageProps) {
  const resolvedSearchParams = use(searchParams)
  const id = resolvedSearchParams.id as string | undefined

  // Hooksは条件分岐の前に呼び出す必要があるため、常に呼び出す
  const charaChipIdNumber = id ? parseInt(id, 10) : 0
  const { data, isLoading, error } = useCharachipQuery(charaChipIdNumber)

  if (!id) {
    return (
      <div className="container mx-auto px-3 md:px-6 py-8">
        <div className="text-center py-12">
          <p className="text-red-600 mb-4">キャラチップIDが指定されていません</p>
  ...
  if (isNaN(charaChipId)) {
    return (
      <div className="container mx-auto px-3 md:px-6 py-8">
        <div className="text-center py-12">
          <p className="text-red-600 mb-4">無効なキャラチップIDです</p>
          <Link href="/charachip-list">
            <Button variant="outline">キャラチップ一覧に戻る</Button>
          </Link>
        </div>
