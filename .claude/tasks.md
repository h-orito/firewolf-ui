# FIREWOLF Nuxt4移行 タスク一覧

## 概要
FIREWOLF Nuxt4移行プロジェクトの全タスクをフェーズ別に管理します。

## Phase 0: 準備・整理作業 (Week 0)

### 🗂️ 現行コードの整理・退避
- [ ] 現行ファイルの一時退避（作業の邪魔にならないよう）
  - [ ] `mkdir .old-nuxt2` で退避ディレクトリ作成
  - [ ] 主要な旧ファイル群を .old-nuxt2/ に移動
  - [ ] 必要に応じて参照できるよう保管

### 📋 現行システムの詳細分析・記録
- [ ] 使用中のBuefyコンポーネント一覧作成
  - [ ] 全.vueファイルでのBuefyコンポーネント使用状況調査（grep検索）
  - [ ] カスタマイズ内容の詳細記録（コード解析ベース）
  - [ ] 依存するpropsやeventsの洗い出し
- [ ] 現行の状態管理構造記録
  - [ ] Vuex store構造の詳細分析
  - [ ] actions, mutations, getters の使用パターン記録
- [ ] API通信パターンの分析
  - [ ] axios使用箇所の特定
  - [ ] エラーハンドリングパターンの記録
  - [ ] インターセプター設定の保存

### 🔧 新環境構築の準備
- [ ] 退避完了後の作業スペース確保
- [ ] 必要な設定ファイルの参照準備
- [ ] 既存の重要な設定値の記録・保存

## Phase 1: 環境構築 (Week 1)

### 🔍 事前調査・検証
- [ ] @nuxt/uiのNuxt 4対応状況確認
  - [ ] 公式ドキュメントで対応状況確認
  - [ ] GitHub Issues/Discussionsで既知問題確認
  - [ ] Nuxt 4 RC版でサンプル動作検証
  - [ ] 継続可否の最終判断

### 🏗️ Nuxt 4プロジェクト構築
- [ ] Node.js 22環境準備
- [ ] 現在のディレクトリでNuxt 4プロジェクト作成
  - [ ] `nuxi init .` でNuxt 4プロジェクト初期化（上書き）
  - [ ] 必要に応じて退避したファイルから設定値を移植
- [ ] 基本ディレクトリ構造作成

### ⚙️ 開発環境設定
- [ ] TypeScript 5.7+設定
- [ ] ESLint 9 + Prettier 3設定
- [ ] VSCode設定ファイル作成
- [ ] .gitignore設定

### 🔧 開発コマンド設定・検証
- [ ] package.json スクリプトの設定
  - [ ] `"lint": "eslint ."` の設定
  - [ ] `"format": "prettier --write ."` の設定
  - [ ] `"type-check": "nuxt typecheck"` の設定
- [ ] 開発コマンドの動作確認
  - [ ] `pnpm lint` の実行確認
  - [ ] `pnpm format` の実行確認
  - [ ] `pnpm type-check` の実行確認
  - [ ] 全コマンドがエラーなく動作することを確認

### 🎨 @nuxt/ui + Tailwind CSS導入
- [ ] @nuxt/ui インストール・基本設定
- [ ] Tailwind CSS設定ファイル作成
- [ ] 現行カスタム色の移植
  - [ ] assets/sass/firewolf.scssから色定義抽出
  - [ ] Tailwind設定にカスタムカラー定義
  - [ ] 人狼ゲーム固有色の設定
- [ ] サンプルコンポーネントで動作確認

### 📋 Week 0完了基準
- [ ] 旧ファイルの整理・退避完了
- [ ] Buefyコンポーネント使用状況の完全リスト化（コードベース調査）
- [ ] 重要な設定値・環境変数の記録完了
- [ ] 新環境構築の準備完了

### 📝 備考：現行アプリの動作確認について
- Node14以前の環境のため、現行アプリの動作確認は実施不可
- デザイン確認が必要な場合は、ユーザーに本番環境でのスクリーンショット撮影を依頼
- 基本的にはコードベースでの分析・実装を優先

### 📋 Week 1完了基準  
- [ ] Nuxt 4 + @nuxt/ui環境が正常動作
- [ ] 開発サーバーが起動する（localhost:3000）
- [ ] @nuxt/uiコンポーネントが表示される
- [ ] 現行カスタム色がTailwindで適用される
- [ ] 開発コマンドが全て正常動作
  - [ ] `pnpm lint` - エラーなし
  - [ ] `pnpm format` - 正常実行
  - [ ] `pnpm type-check` - TypeScriptエラーなし

---

## Phase 2: 基盤移行 (Week 2-3)

### 🔐 Firebase認証移行
- [ ] Firebase v10設定の移行
- [ ] Firebase設定ファイル作成
- [ ] 環境変数設定（.env）
- [ ] VueFire導入・設定
- [ ] 認証状態管理のPinia実装
- [ ] 認証ミドルウェアの実装
- [ ] ログイン・ログアウト機能実装
- [ ] 既存ユーザーデータとの互換性確認

### 🗄️ Pinia状態管理実装
- [ ] Pinia導入・基本設定
- [ ] auth store実装（Vuexから移行）
- [ ] village store実装（Vuexから移行）
- [ ] village-settings store実装（Vuexから移行）
- [ ] 既存Vuex状態との互換性確認
- [ ] リアルタイムデータバインディング実装

### 📡 API通信層実装
- [ ] $fetchベースのAPI client実装
- [ ] エラーハンドリング機能実装
- [ ] リクエスト・レスポンスインターセプター実装
- [ ] 既存API エンドポイントとの互換性確認
- [ ] API通信テスト実装

### 📋 Week 2-3完了基準
- [ ] ユーザーログイン・ログアウトが動作
- [ ] Firestoreリアルタイム同期が動作
- [ ] 既存APIとの通信が成功
- [ ] 認証状態が正しく管理される

---

## Phase 3: UIコンポーネント移行 (Week 4-7)

### Week 4: 共通コンポーネント
- [ ] Loading コンポーネント実装
  - [ ] 現行Buefyコンポーネントの詳細分析（コードベース）
  - [ ] ⚠️ デザイン確認必要時はユーザーに本番スクリーンショット依頼
  - [ ] @nuxt/ui + Tailwindで再実装
  - [ ] アニメーション・状態管理
- [ ] Modal コンポーネント実装
  - [ ] 現行b-modalの使用パターン分析
  - [ ] @nuxt/uiのUModalで再実装
- [ ] Toast/Notification コンポーネント実装
- [ ] Button コンポーネント群実装
  - [ ] 現行b-buttonのvariant・size分析
  - [ ] カスタムスタイルの移植
- [ ] Form要素実装（Input, Select, Checkbox等）
  - [ ] 現行b-field、b-input等の分析
  - [ ] vee-validate v4との統合

### Week 5: レイアウトコンポーネント  
- [ ] DefaultLayout実装
- [ ] VillageLayout（村専用レイアウト）実装
- [ ] Navigation コンポーネント実装
- [ ] Header コンポーネント実装
- [ ] Footer コンポーネント実装
- [ ] Sidebar コンポーネント実装

### Week 6: 人狼ゲーム固有コンポーネント
- [ ] CharaImage（キャラクター画像）コンポーネント実装
- [ ] MessageCard（発言表示）コンポーネント実装
  - [ ] 役職別発言色の実装
  - [ ] ダークモード対応
- [ ] ActionForm（アクション実行フォーム）実装
- [ ] VillageStatus（村の状態表示）実装
- [ ] PlayerList（参加者一覧）実装

### Week 7: デザイン調整・検証
- [ ] デザイン再現度の確認
  - [ ] ⚠️ 必要に応じてユーザーに本番環境スクリーンショット依頼
  - [ ] コードベースでの比較検証（CSS、クラス名等）
- [ ] レスポンシブデザイン調整
- [ ] ダークモード対応検証
- [ ] アクセシビリティ確認・改善
- [ ] コンポーネントスタイルガイド作成

### 📋 Week 4-7完了基準
- [ ] 現行UIと視覚的に同一（コードベース検証 + 必要時スクリーンショット確認）
- [ ] レスポンシブデザインが動作
- [ ] アクセシビリティ基準を満たす
- [ ] すべてのコンポーネントが正常動作

---

## Phase 4: 機能移行 (Week 8-11)

### Week 8: 基本ページ実装
- [ ] トップページ（`/`）実装
  - [ ] 村一覧表示機能
  - [ ] プレイヤー統計表示
  - [ ] キャラチップ情報表示
- [ ] ルール・ドキュメントページ実装
- [ ] FAQ・Aboutページ実装
- [ ] 静的ページのSEO対応

### Week 9: 村一覧・検索機能
- [ ] 村一覧ページ（`/village-list`）実装
- [ ] フィルタリング機能実装
- [ ] ソート機能実装  
- [ ] ページネーション実装
- [ ] リアルタイム更新機能実装
- [ ] 検索パフォーマンス最適化

### Week 10: 村作成機能
- [ ] 村作成ページ（`/create-village`）実装
- [ ] 村作成フォーム実装
- [ ] vee-validate v4によるバリデーション実装
- [ ] 村設定プレビュー機能実装
- [ ] キャラチップ選択機能実装
- [ ] 村作成API連携

### Week 11: 村機能・プレイヤー機能
- [ ] 村ページ（`/village`）実装
  - [ ] メッセージ表示・投稿機能
  - [ ] アクション実行機能
  - [ ] 日付切り替え機能
  - [ ] リアルタイム更新機能
- [ ] プレイヤー戦績（`/player-record`）実装
- [ ] 設定ページ（`/setting`）実装
- [ ] キャラチップ関連ページ実装

### 📋 Week 8-11完了基準
- [ ] 全機能が現行と同等に動作
- [ ] Class Components → Composition API移行完了
- [ ] 機能テストが全て通過
- [ ] エラーが発生しない

---

## Phase 5: 最適化・本番準備 (Week 12-13)

### Week 12: パフォーマンス最適化
- [ ] バンドルサイズ分析・最適化
- [ ] 遅延ローディング実装
- [ ] 画像最適化実装
- [ ] キャッシュ戦略実装
- [ ] Lighthouse スコア測定・改善
- [ ] PWA対応（@vite-pwa/nuxt）
- [ ] Service Worker実装
- [ ] オフライン対応実装

### Week 13: テスト・デプロイ準備
- [ ] Vitest単体テスト実装
  - [ ] コンポーネントテスト
  - [ ] ストアテスト  
  - [ ] ユーティリティ関数テスト
- [ ] Playwright E2Eテスト実装
  - [ ] 主要フローテスト
  - [ ] 回帰テスト
- [ ] 環境変数設定
- [ ] ビルド設定最適化
- [ ] CI/CD設定
- [ ] エラー監視設定
- [ ] 本番環境デプロイテスト

### 📋 Week 12-13完了基準
- [ ] Lighthouse Score 90以上
- [ ] テストカバレッジ 80%以上
- [ ] 本番環境で正常動作
- [ ] パフォーマンス要件達成

---

## 🚨 リスク対応タスク

### 高優先度リスク対応
- [ ] @nuxt/ui非対応時の代替計画実行準備
- [ ] Firebase認証移行のロールバック計画作成
- [ ] デザイン再現困難時の対応策準備

### 継続監視タスク
- [ ] 週次リスク評価実施
- [ ] パフォーマンス継続監視
- [ ] 品質メトリクス測定

---

## 📝 ドキュメント更新タスク

### 開発中の更新
- [ ] 技術課題・解決策の記録
- [ ] APIドキュメントの更新
- [ ] コンポーネント仕様書の作成

### 完了時の更新  
- [ ] 最終的な技術選定結果の記録
- [ ] 移行完了レポートの作成
- [ ] 運用・メンテナンス手順書の作成

---

## 🎯 品質チェックポイント

### 各フェーズ共通
- [ ] 開発コマンド品質チェック
  - [ ] `pnpm lint` - ESLint警告・エラーなし
  - [ ] `pnpm format` - コードフォーマット統一
  - [ ] `pnpm type-check` - TypeScript型エラーなし
- [ ] 単体テスト通過
- [ ] デザインレビュー合格
- [ ] パフォーマンス基準達成

### 最終チェック
- [ ] 全機能動作確認
- [ ] クロスブラウザ動作確認
- [ ] モバイル対応確認
- [ ] アクセシビリティ基準達成
- [ ] SEO設定確認

---

## 🔄 切り替え・統合タスク

### 段階的切り替え準備
- [ ] 機能別切り替えタイミングの計画
  - [ ] 静的ページから順次切り替え
  - [ ] 重要機能は最後に切り替え
- [ ] ドメイン・URL切り替え計画
- [ ] データベース・API切り替え計画
- [ ] ユーザーへの告知・移行案内

### 最終統合作業
- [ ] 新システムの本格運用開始
- [ ] 現行システムの段階的停止
- [ ] 旧ファイルの整理・アーカイブ
- [ ] ドキュメント・README更新

## ⏰ 緊急時対応タスク

### @nuxt/ui使用不可時
- [ ] Gitブランチでのロールバック
- [ ] Tailwind CSS + Headless UI構成に即座切り替え
- [ ] 独自コンポーネント実装計画の実行
- [ ] スケジュール調整

### 重大バグ発生時
- [ ] 問題の特定・分析
- [ ] 影響範囲の調査
- [ ] 修正方針の決定・実行
- [ ] 再発防止策の実装

### データ・設定の緊急復旧
- [ ] 退避ディレクトリからの設定復旧
- [ ] 環境変数・設定ファイルの復旧
- [ ] ユーザーデータの整合性確認

---

**最終更新**: 2025年1月  
**総タスク数**: 120+項目  
**推定工数**: 約3.5ヶ月（Phase 0含む14週間）

## 📁 作業ディレクトリ構造

```
firewolf-ui/                 # 作業ディレクトリ (feature/nuxt4ブランチ)
├── .old-nuxt2/             # 退避した旧ファイル群
│   ├── nuxt.config.ts      # 旧Nuxt 2設定
│   ├── package.json        # 旧依存関係
│   └── components/         # 旧コンポーネント（参照用）
├── nuxt.config.ts          # 新Nuxt 4設定
├── package.json            # 新しい依存関係
├── components/             # 新@nuxt/uiコンポーネント
└── ...
```