...
- [x] `components/pages/village/action/creator/SettingsLink.vue` の作成
  - 村設定変更画面へのリンク
  - プロローグ中のみ表示
...
**プロローグ中の村設定を村主が変更するページ**

- [x] ページ基盤・ルーティング
  - [x] `app/pages/setting.vue` ページ作成（クエリパラメータ `id` で villageId 取得）
  - [x] 村情報取得・フォーム初期化
  - [x] アクセス権限チェック（村主のみ）
- [x] 設定フォームコンポーネント
  - [x] 基本設定（村名、開始日時、沈黙時間）
  - [x] ダミーキャラ発言（1日目発言のみ変更可能）
  - [x] 編成設定（組織構成、役欠け）
  - [x] 詳細ルール（記名投票、役職希望、見学、突然死、時短希望、連続護衛）
  - [x] 発言制限（各発言タイプの回数・文字数）
  - [x] 参加パスワード
  - [x] RP設定（年齢制限、墓下公開、秘話、アクション）
- [x] 確認・保存機能
  - [x] バリデーション実装
  - [x] 確認モーダル表示
  - [x] API連携（confirm → setting）
  - [x] 保存後の村ページへのリダイレクト

##### 注意事項

- 村作成ページ（`/create-village`）と多くのフォームコンポーネントを共有可能
- キャラチップ関連は村作成後は変更不可のため、表示のみまたは非表示
- 参加パスワードは毎回空で表示される仕様
...
### Week 14: パフォーマンス最適化 ✅ 完了

- [x] バンドルサイズ分析・最適化
- [x] 遅延ローディング実装
- [x] 画像最適化実装
- [x] キャッシュ戦略実装
- [x] Lighthouse スコア測定・改善
...
- [x] 環境変数設定（既存設定を使用）
- [x] ビルド設定最適化（netlify.toml作成）
- [x] CI/CD設定（Netlify自動デプロイ）
- [x] エラー監視設定（現状維持）
- [ ] 本番環境デプロイテスト
