...
**村画面ルーティング**
- **URL形式**: `/village?id={village_id}`
- **実装**: `app/village/page.tsx` でクエリパラメータから村IDを取得
- **例**: `/village?id=123` → 村ID 123の村画面を表示
...
export default async function VillagePage({
  ...
  const villageId = searchParams.id
  // 村データの取得とレンダリング
}
```

**キャラチップルーティング**
- **URL形式**: `/charachip?id={charachip_id}`
- **実装**: `app/charachip/page.tsx` でクエリパラメータからキャラチップIDを取得
- **例**: `/charachip?id=456` → キャラチップID 456の詳細画面を表示

```typescript
// app/charachip/page.tsx
export default async function CharachipPage({
  searchParams,
}: {
  searchParams: { id?: string }
}) {
  const charachipId = searchParams.id
  // キャラチップデータの取得とレンダリング
}
```

**利点**
- Next.js App Router の標準的なクエリパラメータ処理
- 動的ルートセグメントより簡潔な実装
- パラメータの有無による条件分岐が容易
...
- **表示パターン**:
  - 未展開状態: 右向き矢印 (`→`) を使用
  - 展開済み状態: 下向き矢印 (`↓`) を使用
- **視覚的一貫性**: 全てのアコーディオンコンポーネントで統一した表示ルール
- **実装方針**:
  - Font Awesome の `fa-chevron-right` (未展開) と `fa-chevron-down` (展開済み)
  - または Unicode記号 `→` (U+2192) と `↓` (U+2193) を使用
  - CSS transition による滑らかな矢印回転アニメーション
- **適用対象**:
  - サイドバーの展開可能セクション
  - CollapsiblePanel コンポーネント
  - その他の展開/縮小機能付きUI要素
- **アクセシビリティ**: `aria-expanded` 属性との連動表示
...
**村情報モーダル設計**
- **表示内容**: パスワード以外の村設定を一覧表示
  - 村基本情報（村名、村建て、キャラセット等）
  - 参加制限設定（参加パスワード、見学設定等）
  - ルール設定（投票・能力・コミット等の設定）
  - メッセージ設定（発言制限回数・文字数等）
- **UI設計**: 
  - カテゴリ別セクション分け
  - 読みやすいラベル・値のペア表示
  - 設定値のハイライト表示
  - **キャラチップ新規タブリンク**: キャラチップ項目は `/charachip/{charachip_id}` への新規タブリンクとして表示
  - **編成設定詳細表示**: プロローグ中は人数ごとの編成を全部表示（例: `10人: 狼狼占霊狩狂村村村村\n11人: 狼狼占霊狩狂村村村村村...`）
- **アクション**: 閉じるボタン、背景クリックで閉じる、キャラチップクリックで新規タブ開く
- **レスポンシブ**: モバイル対応のスクロール可能レイアウト

**参加者一覧展開/縮小設計**
- **デフォルト状態**: 縮小状態（「参加者一覧」ボタンのみ表示）
- **展開状態**: 参加者リストを表示
- **トグル機能**: ボタンクリックで状態を切り替え
- **アニメーション**: スムーズな展開・縮小アニメーション
- **状態保持**: 同一セッション内での状態記憶（Cookie保存は行わない）

**メモ機能モーダル設計**
- **メモ管理**: 最大3個のメモを作成・編集・削除
- **エディタ機能**: 
  - リアルタイム文字数・行数カウンター表示
  - テキストエリアでの複数行入力
  - メモタイトル設定（任意）
- **永続化**: Cookie による30日間の保存
- **UI設計**:
  - タブ形式で3つのメモを切り替え表示
  - 各タブに「メモ1」「メモ2」「メモ3」のラベル
  - アクティブなタブのハイライト表示
- **バリデーション**: 文字数制限の設定・表示
- **アクション**: 保存、削除、キャンセル、閉じる

**ユーザー設定モーダル設計**
- **設定カテゴリ**: 
  - 表示設定（発言表示、メッセージフィルタ設定等）
  - 操作設定（確認ダイアログ、ショートカット等）
  - その他設定（個人抽出方式等）
- **UI設計**:
  - カテゴリ別タブまたはセクション分け
  - トグルスイッチによるON/OFF設定
  - ドロップダウンによる選択肢設定
- **永続化**: Cookie による1年間の保存
- **リアルタイム反映**: 設定変更の即座反映
- **リセット機能**: デフォルト設定に戻す機能
...
**サイドバー広告レイアウト設計**
- **中央揃え実装**: `text-center` または `flex justify-center` によるセンタリング
- **レスポンシブ対応**: モバイルでも統一した中央配置を維持
- **マージン調整**: 上下の要素との適切な間隔調整
...
**現在の課題**
- サイドバーに管理者メニューとデバッグメニューが配置されている
- サイドバーは情報表示がメインの領域であり、アクション系機能との混在が発生

**新設計方針**
- **アクションエリアへの統合**: 管理者・デバッグメニューをメインコンテンツのGameActionsエリアに移動
- **論理的グルーピング**: 他のアクション機能（発言、能力行使等）と同じ位置に配置
- **権限ベース表示**: SituationAsParticipant による表示制御を維持
- **UI一貫性**: 既存のアクションパネルと同じデザイン言語を使用

**実装設計**
```typescript
// GameActions コンポーネントに管理者・デバッグアクションを統合
const AdminActions = ({ situation }: { situation: SituationAsParticipant }) => {
  if (!situation.isAdmin && !situation.isDebugger) return null;
  
  return (
    <CollapsiblePanel title="管理機能" defaultOpen={false}>
      {situation.isAdmin && <AdminMenu />}
      {situation.isDebugger && <DebugMenu />}
    </CollapsiblePanel>
  );
};
```
...
**サイドバーの簡素化**
- 管理者・デバッグメニュー削除により情報表示に特化
- VillageInfo、ParticipantList、MemoSection、UserSettings、Advertisement のみに
- よりクリーンで情報整理されたサイドバーレイアウト
...
**課題認識**
- 「ログイン中」テキストが冗長でスペースを無駄に消費
- サイドバーの限られたスペースの効率的活用が必要
- ユーザビリティを損なわずにより簡潔な表示を実現

**新デザイン方針**
- **コンパクトレイアウト**: `{nickname} {マイページボタン} {ログアウトボタン}` の横並び配置
- **不要テキスト削除**: 「ログイン中」の表示文字列を削除
- **視覚的階層**: ニックネームを主要情報として強調表示
- **アクションボタン**: マイページ・ログアウトを小サイズボタンで配置

**実装設計**
```typescript
// LoginStatus コンポーネントの新レイアウト設計
const LoginStatus = ({ user }: { user: User | null }) => {
...
**ページ初期表示テスト**
- 全ページの初期表示が正常に完了することを確認
- h1要素の存在確認（基本的な画面表示の確認）
- JavaScript エラーの発生有無を検証
- ローディング状態の適切な完了を確認
...
│   ├── village-list.spec.ts # 村一覧ページテスト
│   ├── village.spec.ts      # 村ページテスト
│   └── auth.spec.ts         # 認証ページテスト
├── utils/                   # テストユーティリティ
└── fixtures/                # テストデータ
```
