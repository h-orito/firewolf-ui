...
- **PC版ヘッダーのリンク削除**:
  ...
  - よりシンプルでクリーンなヘッダーデザインに変更
  - これらのリンクはハンバーガーメニューとフッターからアクセス可能
...
#### キャラチップ詳細画面の実装設計
- **画像表示の修正**:
  - キャラクター画像が適切に表示されるよう修正
  - Next.js Image componentのexternal domain設定
  - 画像読み込みエラー時の適切なフォールバック処理
- **表示項目の最適化設計**:
  - 表情区分名（face_type_name）の表示を削除
  - 表情種類数の表示を削除
  - よりシンプルで使いやすいUIを提供
- **パフォーマンス改善設計**:
  - キャラクター数が多い場合のlazy loading実装
  - Intersection Observer APIまたはReact.lazyを活用
  - 初期表示速度とユーザー体験の最適化
- **リンクテキストの改善**:
  - 「素材サイトを見る」を「作者 HP」に変更
  - より明確で理解しやすい表現への変更
- **名前変更可否表示の実装**:
  - キャラチップ設定から名前変更可能フラグを取得
  - 「名前変更: 可能/不可」として視覚的に表示
  - ユーザーがキャラチップの制約を事前に理解できるよう情報提供
- **画像表示の最適化設計**:
  - キャラクター画像のobject-coverクラスを削除（切り取り無し）
  - roundedクラスを削除（角丸無し）
  - API responseのsizeプロパティから画像の本来のサイズを取得して適用
  - 画像の縦横比を保持した自然な表示を実現

#### トップページ村カード表示の改善設計
- **v1スタイルの村カード実装**:
  - 村カードの背景画像上にテキスト情報を重ね表示
  - 半透明の背景色またはテキストシャドウで視認性を確保
  - レスポンシブデザインでの適切な文字サイズとレイアウト調整
- **カード全体のリンク化**:
  - 開催中の村カード全体をクリッカブルエリアとして実装
  - ホバー時の視覚的フィードバック（影やボーダーの変化）
  - アクセシビリティに配慮したフォーカス状態の表示
- **実装方針**:
  - VillageCardコンポーネントの修正
  - 既存の情報レイアウトを背景画像上のオーバーレイ表示に変更
  - CSS-in-JSまたはTailwindクラスでのスタイリング実装
  - カード全体を`<Link>`コンポーネントでラップ
...
- **トップページへの実装**:
  ...
  - 投げ銭用のモーダルコンポーネントを新規作成
  - トップページに投げ銭ボタンまたはリンクを配置
  - shadcn/uiのDialogコンポーネントを活用したモーダル実装
- **投げ銭内容の設計**:
  - v1と同じ投げ銭サービスへのリンクを配置
  - 投げ銭の説明文を含める
  - わかりやすいUIで支援方法を提示
